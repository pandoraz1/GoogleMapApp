...
public class MapsActivity extends FragmentActivity implements GoogleMap.OnMapLongClickListener, LocationListener {
    ...
    private void setUpMap() {
        LatLng BKK = new LatLng(13.755949,100.533547);
        LatLng HK = new LatLng(22.279649,114.177310);
        LatLng BKK2 = new LatLng(16.821123,100.266136);

        mMap.addMarker(new MarkerOptions().position(BKK).title("Bangkok"));
        mMap.addMarker(new MarkerOptions().position(HK).title("Hong Kong"));
        mMap.addMarker(new MarkerOptions().position(BKK2).title("Phitsanulok"));
...
*/
    ...
    public void onLocationChanged(Location location) {
        LatLng OTHEPLACE = new LatLng(location.getLatitude(),location.getLongitude());
        mMap.addMarker(new MarkerOptions().position(OTHEPLACE).title("Place "+ ++Place));

        CameraUpdate start_map = CameraUpdateFactory.newLatLng(OTHEPLACE);
        mMap.moveCamera(start_map);
        ...
        if(lPlace != null) {
            mMap.addPolyline(new PolylineOptions().add(lPlace).add(OTHEPLACE));
        }
